...
describe('ParagraphChangeTrackerExtension', () => {
  describe('mark-only edits (selective save)', () => {
    ...
    test('does not crash when a RemoveMarkStep is followed by a shrinking ReplaceStep', () => {
      // Regression: accept-tracked-replacement emits
      //   [RemoveMarkStep(insMark, insFrom..insTo),
      //    ReplaceStep(delFrom..delTo, empty)]
      // where the replace shrinks the doc and the mark step's `to`
      // ends up past the final doc end. Without remapping, the
      // plugin walked `tr.doc.nodesBetween(from, to)` with stale
      // coords and crashed on `undefined.nodeSize`.
      let state = createState([
        { text: 'AAAA', paraId: 'P1' },
        { text: 'BBBB', paraId: 'P2' },
      ]);
